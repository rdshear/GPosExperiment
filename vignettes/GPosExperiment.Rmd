...
A `HRseqData` object contains the information necessary to add a sample to a GPosExperiment object. At a minimum, it contains a string that identifies the sample and a GPos object containing the scores from that sample. The object may also contain a a `GenomeInfoDb::Seqinfo` object or the name of an NCBI registered genome. 
...
Our two samples are fragments from samples deposited in SRA. We construct a list of HRseqData objects.
